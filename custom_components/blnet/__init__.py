...
REQUIREMENTS = ["git+https://github.com/sanzinger/pyblnet.git@fix_ta_client_fetch"]
        ...
        for node in nodes:
            ...
            for domain in ["analog", "speed", "power", "energy"]:
                ...
                for key, sensor in data.get(domain, {}).items():
                    attributes = {}
                    entity_id = "{} {} {}_{}".format(DOMAIN, domain, node, key)
                    attributes["value"] = sensor.get("value")

                    attributes["unit_of_measurement"] = sensor.get(
                        "unit_of_measurement", UNIT[domain]
                    )
            ...
            for key, sensor in data.get("digital", {}).items():
                attributes = {}
                entity_id = "{} digital {} {}".format(DOMAIN, node, key)
            ...
            for domain in ["analog", "speed", "power", "energy"]:
                for sensor_id in data[domain]:
                    name = "{} {} {}_{}".format(DOMAIN, domain, node, sensor_id)
                    if name in self.sensors:
                        continue
                    self.sensors.add(name)
                    _LOGGER.info(
                        "Discovered {} sensor {} in use, adding".format(
                            domain, sensor_id
                        )
                    )
                    i += 1
            ...
            for sensor_id in data["digital"]:
                name = "{} digital {} {}".format(DOMAIN, node, sensor_id)
                if name in self.sensors:
                    continue
                self.sensors.add(name)
                _LOGGER.info(
                    "Discovered digital sensor {} in use, adding".format(sensor_id)
                )
                i += 1
                disc_info = {
                    "name": name,
